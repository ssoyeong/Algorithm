...
public class BJ16637 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        dfs((long)numbers.get(0), 0);
        System.out.println(ans);
    }

    private static void dfs(long result, int index) {

        if(index == numbers.size() - 1) {
            ans = Long.max(ans, result);
            return;
        }

        // (a ? b) ? c 연산인 경우, (a ? b) 계산하기
        long calculated = calculate(result, numbers.get(index + 1), operators.get(index));
